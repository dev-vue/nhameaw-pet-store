...
export function SearchCategoryModal({ animalType, name, open, onClose }: SearchCategoryModalProps) {
    ...
    const handleSubcategoryClick = async (subcategory: ProductCategory) => {
        if (subcategory.list && subcategory.list.length > 0) {
            ...
            push(`/search?category=${encodeURIComponent(subcategory.id)}&categorytext=${encodeURIComponent(subcategory.detail)}`);
            onClose();
        }
    };
    ...
    const handleViewAllProducts = () => {
        ...
        push(`/search?category=${encodeURIComponent(category ?? '')}&categorytext=${encodeURIComponent(searchtext)}`);
        onClose();
    };
